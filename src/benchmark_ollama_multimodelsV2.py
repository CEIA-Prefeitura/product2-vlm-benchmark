...
# --- Configuração ---
MODEL_NAMES_TO_TEST = os.getenv("OLLAMA_BENCHMARK_MODELS", "gemma3:4b,gemma3:12b,gemma3:27b").split(',') # Modelos VLMs a serem utilizados no Ollama
TEMPERATURES_TO_TEST = [float(t) for t in os.getenv("BENCHMARK_TEMPERATURES", "0.2,0.4").split(',')] # Ex: [0.1, 0.5, 0.8]
DATASET_PATH = Path(os.getenv("BENCHMARK_DATASET_PATH", "./data/CSVs/91_casas_terreas_anotadas.csv")) # Caminho para o arquivo CSV contendo o dataset de imóveis a serem analisados.
IMAGES_BASE_DIR = Path(os.getenv("BENCHMARK_IMAGES_DIR", "./data/imagens/images")) # Diretório onde as imagens dos imóveis estão localizadas.
...
PROMPTS_DIR = Path(os.getenv("BENCHMARK_PROMPTS_DIR_GERAL", "./prompts/prompts_especialistas/casa_terrea")) # Diretório contendo os arquivos de texto para os prompts do sistema e humano.
TIPO_PROMPT_LABEL = "analise_imovel_casa_terrea_v1" # Rótulo para o tipo/versão do conjunto de prompts utilizado.
        ...
        if "error" in api_response_data:
            ...
            if base_result["raw_model_output"]:
                ...
                if parsed_json and not parsing_error: 
                    ...
                    logger.warning(f"Error: {parsing_error}")
...
def find_property_image_dir(base_dir: Path, property_id_search_term: str, property_type_search_term: str) -> Optional[Path]:
    ...
    patterns_to_try = [property_id_search_term, f"{property_type_search_term}/{property_id_search_term}"]
...
def run_multiconfig_json_generation() -> None:
    ...
    for model_name_current in MODEL_NAMES_TO_TEST:
        ...
        try:
            if not validate_model(model_name_current): # Valida se o modelo configurado está disponível no Ollama (FUNÇÃO DO 'ollama_connector.py')
        ...
        for temp_current in TEMPERATURES_TO_TEST:
            ...
            output_final_geracao_run = {
                "metadata_geracao_run": {
                    "timestamp_geracao_inicial_run": overall_start_time_iso.isoformat(), # Pode ser um timestamp específico do início deste run
                    "timestamp_geracao_final_run": datetime.now(timezone.utc).isoformat(),
                    "timestamp_final_run": (datetime.now(timezone.utc) - overall_start_time_iso).total_seconds(),
