...
'''
Script para a geração de métricas e gráficos - VERSÃO CORRIGIDA
'''
...
DEFAULT_DATASET_CSV_PATH = PROJECT_ROOT / "data" / "CSVs" / "91_casas_terreas_anotadas.csv" # Seu CSV de ground truth
...
def calculate_correct_weighted_averages(metrics_for_csv_plot, aggregated_metrics_results):
    """
    Calcula médias ponderadas corretas considerando o support de cada categoria.
    Calcula F1-score manualmente para garantir consistência matemática.
    
    Args:
        metrics_for_csv_plot: Dict com métricas por categoria
        aggregated_metrics_results: Dict com resultados agregados incluindo support
    
    Returns:
        Dict com métricas médias ponderadas corretas ou None se não foi possível calcular
    """
    ...
    logger.info("Calculando médias ponderadas corretas com F1 manual...")
    
    for category_name, mets_dict in metrics_for_csv_plot.items():
        if "error" not in mets_dict and mets_dict.get("accuracy") is not None:
            # Buscar o support total da categoria nos resultados agregados
            safe_key = category_name.replace(' ', '_').replace('ç', 'c').replace('ã', 'a').replace('á', 'a').replace('é', 'e')
            
            category_metrics = aggregated_metrics_results.get(safe_key, {})
            if "metrics_per_class" in category_metrics:
                # Somar o support de todas as classes desta categoria
                category_support = sum(
                    class_metrics.get("support_ground_truth", 0) 
                    for class_metrics in category_metrics["metrics_per_class"].values()
                )
                
                if category_support > 0:
                    weighted_accuracy_sum += mets_dict["accuracy"] * category_support
                    weighted_precision_sum += mets_dict.get("precision_weighted", 0.0) * category_support
                    weighted_recall_sum += mets_dict.get("recall_weighted", 0.0) * category_support
                    total_samples += category_support
                    categories_used += 1
                    
                    logger.debug(f"Categoria '{category_name}': support={category_support}")
                    logger.debug(f"  - accuracy={mets_dict['accuracy']:.4f}")
                    logger.debug(f"  - precision={mets_dict.get('precision_weighted', 0.0):.4f}")
                    logger.debug(f"  - recall={mets_dict.get('recall_weighted', 0.0):.4f}")
                    logger.debug(f"  - f1_sklearn={mets_dict.get('f1_score_weighted', 0.0):.4f}")
                else:
                    logger.warning(f"Categoria '{category_name}': support=0, ignorando para média ponderada")
            else:
                logger.warning(f"Categoria '{category_name}': metrics_per_class não encontrado")
    
    if total_samples > 0:
        # Calcular médias ponderadas
        accuracy_avg = weighted_accuracy_sum / total_samples
        precision_avg = weighted_precision_sum / total_samples
        recall_avg = weighted_recall_sum / total_samples
        
        # Calcular F1-score manualmente usando a fórmula correta
        if precision_avg + recall_avg > 0:
            f1_manual = 2 * (precision_avg * recall_avg) / (precision_avg + recall_avg)
        else:
            f1_manual = 0.0
        
        logger.info(f"Médias calculadas sobre {categories_used} categorias e {total_samples} amostras:")
        logger.info(f"  - Accuracy: {accuracy_avg:.6f}")
        logger.info(f"  - Precision: {precision_avg:.6f}")
        logger.info(f"  - Recall: {recall_avg:.6f}")
        logger.info(f"  - F1 (manual): {f1_manual:.6f}")
        
        result = {
            "accuracy": accuracy_avg,
            "f1_score_weighted": f1_manual,  # Usar F1 calculado manualmente
            "precision_weighted": precision_avg,
            "recall_weighted": recall_avg,
            "total_samples_used": total_samples,
            "num_categories_used": categories_used
        }
        
        return result
    
    logger.warning("Não foi possível calcular médias ponderadas: total_samples = 0")
    return None


def validate_overall_metrics(overall_metrics):
    """
    Valida se as métricas gerais estão matematicamente corretas.
    
    Args:
        overall_metrics: Dict contendo as métricas calculadas
        
    Returns:
        bool: True se as métricas são válidas, False caso contrário
    """
    
    if not overall_metrics:
        logger.warning("Não há métricas para validar")
        return False
    
    accuracy = overall_metrics.get("accuracy")
    recall = overall_metrics.get("recall_weighted") 
    f1 = overall_metrics.get("f1_score_weighted")
    precision = overall_metrics.get("precision_weighted")
    
    if any(metric is None for metric in [accuracy, recall, f1, precision]):
        logger.warning("Algumas métricas estão ausentes, pulando validação")
        return False
    
    tolerance = 1e-10
    is_valid = True
    
    # Para médias weighted corretas, accuracy deve ser igual ao recall
    if abs(accuracy - recall) > tolerance:
        logger.error(f"❌ ERRO MATEMÁTICO: Accuracy ({accuracy:.6f}) != Recall weighted ({recall:.6f})")
        logger.error(f"   Diferença: {abs(accuracy - recall):.10f}")
        is_valid = False
    else:
        logger.info(f"✓ Accuracy = Recall weighted ({accuracy:.6f})")
    
    # Verificar se F1 foi calculado corretamente usando a fórmula manual
    expected_f1 = 2 * (precision * recall) / (precision + recall) if (precision + recall) > 0 else 0.0
    if abs(f1 - expected_f1) > tolerance:
        logger.error(f"❌ ERRO MATEMÁTICO: F1 calculado ({f1:.6f}) != F1 esperado ({expected_f1:.6f})")
        logger.error(f"   Diferença: {abs(f1 - expected_f1):.10f}")
        logger.error(f"   Fórmula: 2 × ({precision:.6f} × {recall:.6f}) / ({precision:.6f} + {recall:.6f}) = {expected_f1:.6f}")
        is_valid = False
    else:
        logger.info(f"✓ F1-score calculado corretamente: {f1:.6f}")
    
    # F1 deve ser <= média aritmética de precision e recall
    arithmetic_mean = (precision + recall) / 2
    if f1 > arithmetic_mean + tolerance:
        logger.error(f"❌ ERRO MATEMÁTICO: F1 ({f1:.6f}) > média aritmética P+R ({arithmetic_mean:.6f})")
        logger.error(f"   F1 deveria ser ≤ {arithmetic_mean:.6f}")
        is_valid = False
    else:
        logger.info(f"✓ F1-score ({f1:.6f}) ≤ média aritmética P+R ({arithmetic_mean:.6f})")
    
    # F1 deve ser >= média harmônica de precision e recall
    harmonic_mean = 2 * (precision * recall) / (precision + recall) if (precision + recall) > 0 else 0.0
    if f1 < harmonic_mean - tolerance:
        logger.error(f"❌ ERRO MATEMÁTICO: F1 ({f1:.6f}) < média harmônica P+R ({harmonic_mean:.6f})")
        is_valid = False
    else:
        logger.info(f"✓ F1-score ({f1:.6f}) = média harmônica P+R ({harmonic_mean:.6f})")
    
    # Verificações adicionais de sanidade
    if not (0 <= accuracy <= 1):
        logger.error(f"❌ Accuracy fora do intervalo [0,1]: {accuracy}")
        is_valid = False
    
    if not (0 <= f1 <= 1):
        logger.error(f"❌ F1-score fora do intervalo [0,1]: {f1}")
        is_valid = False
        
    if not (0 <= precision <= 1):
        logger.error(f"❌ Precision fora do intervalo [0,1]: {precision}")
        is_valid = False
        
    if not (0 <= recall <= 1):
        logger.error(f"❌ Recall fora do intervalo [0,1]: {recall}")
        is_valid = False
    
    if is_valid:
        logger.info("✅ Todas as métricas gerais validadas matematicamente")
...
def run_evaluation(generated_results_file: Path, dataset_csv_path: Path, eval_output_dir: Path):
    ...
    # --- Calcular Médias Gerais das Métricas de Classificação (VERSÃO CORRIGIDA) ---

    overall_average_metrics = {}
    if metrics_for_csv_plot:
        # Usar a função corrigida para calcular médias ponderadas
        corrected_averages = calculate_correct_weighted_averages(metrics_for_csv_plot, aggregated_metrics_results)
        
        if corrected_averages:
            # Mapear para o formato esperado pelo resto do código
            overall_average_metrics[f"accuracy_{model_name_metrics}"] = corrected_averages["accuracy"]
            overall_average_metrics[f"f1_score_weighted_{model_name_metrics}"] = corrected_averages["f1_score_weighted"]
            overall_average_metrics[f"precision_weighted_{model_name_metrics}"] = corrected_averages["precision_weighted"]
            overall_average_metrics[f"recall_weighted_{model_name_metrics}"] = corrected_averages["recall_weighted"]
            overall_average_metrics[f"num_categories_in_{model_name_metrics}"] = corrected_averages["num_categories_used"]
            overall_average_metrics[f"total_samples_{model_name_metrics}"] = corrected_averages["total_samples_used"]
            
            logger.info(f"Métricas médias gerais calculadas corretamente sobre {corrected_averages['num_categories_used']} categorias e {corrected_averages['total_samples_used']} amostras.")
            
            # Validar se as métricas estão matematicamente corretas
            validate_overall_metrics(corrected_averages)
            
        else:
            overall_average_metrics["error"] = "Não foi possível calcular as médias ponderadas corretas."
        ...
        if csv_data_list:
            ...
            if "error" not in overall_average_metrics and overall_average_metrics: # Verifica se as médias foram calculadas
                average_row = pd.DataFrame([{
                    "Categoria": "MÉDIA GERAL (Ponderada)", # Nome atualizado para deixar claro que é ponderada
    ...
    evaluation_summary = {
        "metadata_avaliacao": {
            ...
            "total_categorias_com_media_calculada": overall_average_metrics.get(f"num_categories_in_{model_name_metrics}", 0) if overall_average_metrics else 0,
            "total_samples_used_for_averages": overall_average_metrics.get(f"total_samples_{model_name_metrics}", 0) if overall_average_metrics else 0,
            "averages_calculation_method": "weighted_by_category_support"
        },
